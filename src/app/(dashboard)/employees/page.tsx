...
const log = createLogger("employees-page");

const EMPTY_FORM: RegisterEmployeeRequest = {
  firstName: "",
  lastName: "",
  email: "",
  password: "",
...
export default function EmployeesPage() {
  const { token } = useAuth();
  const [formData, setFormData] = useState<RegisterEmployeeRequest>(EMPTY_FORM);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: `Empleado creado: ${created.email}.`,
      });
    } catch (error) {
      log.error("Error creando empleado", error);
      const text =
        (error as { message?: string })?.message ??
        "No se pudo crear el empleado. Verificá los datos e intentá nuevamente.";
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Alta de Empleado</h1>
        <p className="text-sm text-slate-500 mt-1">
          Registrá un nuevo operario de backoffice. Solo administradores.
        </p>
      </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {message && (
            <div
              className={`sm:col-span-2 p-3 rounded-lg text-sm font-medium ${
